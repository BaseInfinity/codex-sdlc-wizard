...
6. Author-review and commit coherent green slices. Freeze the cumulative candidate for one fresh broad proof and completion review.
   Incremental checkpoint: use affected proof, exact-diff author review, and at most one risk-based reviewer before a coherent green commit. At the completion boundary, review the whole base-to-candidate diff with Sol High and then Fable High. A blocker becomes one bounded corrective delta with targeted proof. A third same-plan correction means stop; human approval may authorize a replan with newly scoped work, not silently extend the exhausted plan. Record ten-delivery pilot outcomes in `benchmarks/review-cadence.csv` before making the cadence permanent.
