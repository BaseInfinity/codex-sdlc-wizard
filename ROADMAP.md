...
- `codex-sdlc-wizard@0.7.6` and `v0.7.6` are the current release target for the live Codex handoff setup flow
...
Purpose: prove the post-`0.7.6` consumer path on real repos and stabilize any reusable wizard bugs without changing the default-use claim early.

Scope:
- run `0.7.6` on 3-5 pilot repos and log results in `benchmarks/pilot-rollout.csv`
...
- avoid speculative backlog churn while `0.7.6` is being consumed on real repos

## Remaining Backlog

After `0.7.6`, the main backlog is:
...
1. Prove the default-use gate on 3-5 pilot repos with `0.7.6`
