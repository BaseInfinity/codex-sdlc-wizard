...
function synchronizeModelProfile(manifest, touchedFiles) {
  ...
  const crossModelReviewer = profile.policy?.cross_model_reviewer;
  if (crossModelReviewer !== undefined) {
    if (!["fable-high", "opus-4.8-xhigh"].includes(crossModelReviewer)) {
      throw new Error(`${profilePath}.policy.cross_model_reviewer is unsupported`);
    }
    next.cross_model_reviewer = crossModelReviewer;
  }
